...
class EnhancedTrackAnalyzer:
    ...
    async def analyze_streaming(
        self, filepath: str
    ) -> Generator[StreamingAnalysisResult, None, None]:
        ...
        try:
            ...
            chunk_size_samples = int(
                self.streaming_chunk_size * 44100
            )  # Assuming 44.1kHz
            ...
            total_chunks = int(
                np.ceil(
                    total_duration
                    / (self.streaming_chunk_size - self.streaming_overlap)
                )
            )
            ...
            for chunk_idx in range(total_chunks):
                ...
                y_chunk, sr = await asyncio.to_thread(
                    librosa.load, filepath, offset=offset, duration=duration, sr=44100
                )
            ...
            yield StreamingAnalysisResult(chunk_index=-1, total_chunks=0, is_final=True)

    async def _analyze_chunk(
        self, y: np.ndarray, sr: int, chunk_idx: int, total_chunks: int, offset: float
        ...
        try:
            ...
            tempo, beats = await asyncio.to_thread(librosa.beat.beat_track, y=y, sr=sr)
    ...
    async def _finalize_streaming_analysis(
        self, filepath: str
    ) -> StreamingAnalysisResult:
        ...
        try:
            ...
            # Create final result
            result = StreamingAnalysisResult(
                chunk_index=-1, total_chunks=-1, is_final=True
            )
    ...
    def analyze_beat_phase(
        self, audio_position: float, beat_times: List[float], bpm: float
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "next_beat": beat_times[current_beat_idx + 1]
            if current_beat_idx < len(beat_times) - 1
            else current_beat + beat_duration,
