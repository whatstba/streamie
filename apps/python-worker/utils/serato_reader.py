...
try:
    ...
    print(f"✅ Mutagen loaded successfully - Ready to parse real Serato data!")
...
class SeratoHotCue:
    ...
    def __init__(self, name: str, time: float, color: str, cue_type: str = 'cue', index: int = 0):
        self.name = name
...
class SeratoReader:
                    ...
                    if tag_name in audio_file.tags:
                        ...
                        if hasattr(tag_data, 'data'):
                            ...
                            for i, cue_data in enumerate(parsed_data.get('cues', [])):
                                ...
                                color = self._serato_color_to_hex(cue_data['color_value'])
                                ...
                                print(f"      📍 Extracted cue: {hot_cue.name} at {time_seconds:.2f}s")
                            ...
                            for i, loop_data in enumerate(parsed_data.get('loops', [])):
                                ...
                                hot_cues.append(hot_cue)
                                print(f"      🔄 Extracted loop: {hot_cue.name} at {start_seconds:.2f}s")
    ...
    def create_demo_cues(self, audio_file_path: str, duration: float) -> List[SeratoHotCue]:
        ...
        if duration > 30:  # Only add demo cues for tracks longer than 30 seconds
            demo_positions = [
                ...
                (max(duration - 32.0, duration * 0.8), "Outro Start", '#ff8000', 'phrase'),
