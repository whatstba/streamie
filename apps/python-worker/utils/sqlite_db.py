...
class SQLiteAdapter:
    ...
    def find_one(self, query: Dict) -> Optional[Dict]:
        ...
        if "filepath" in query:
            cursor.execute("SELECT * FROM tracks WHERE filepath = ?", (query["filepath"],))
        else:
...
class SQLiteCollectionAdapter:
    ...
    def find_one(self, query: Dict = None) -> Optional[Dict]:
        if self.collection_name == "tracks":
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='transition_ratings'")
