...
class DJAgentStreamEnhancer:
        ...
        for pattern in add_patterns:
            ...
            if match:
                if len(match.groups()) >= 3:  # Has artist and BPM
                    return {
                        "title": match.group(1).strip(),
                        "artist": match.group(2).strip() if match.group(2) else "Unknown Artist",
                        "bpm": int(match.group(3)) if match.group(3) else None,
    ...
    def extract_mood_info(self, message: str) -> Optional[Dict]:
        ...
        if match:
            if match.group(1):
                return {"energy": float(match.group(1))}
            elif match.group(2):
    ...
    def process_message(self, message: str) -> Dict:
        ...
        if new_stage and new_stage != self.current_stage:
            ...
            return {
                ...
                "data": {"detected_mood": self.detected_mood} if self.detected_mood else {}
            }
