...
def test_ai_endpoints(sample_track_filepath):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Vibe analysis: {vibe_data['dominant_vibe']} ({vibe_data['energy_level']:.2f} energy)")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Next track suggestion: {suggestion_data['confidence']:.2f} confidence")
        else:
            print(f"❌ Next track suggestion failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Playlist generation: {len(playlist_data['playlist'])} tracks generated")
        else:
            print(f"❌ Playlist generation failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Mixing insights failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
