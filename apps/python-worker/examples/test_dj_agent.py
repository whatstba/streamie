...
async def test_dj_agent():
    ...
    if "error" not in playlist_result:
        ...
        for i, track in enumerate(playlist_result['playlist']):
            print(f"{i+1}. {track.get('title', 'Unknown')} - {track.get('artist', 'Unknown')} ({track.get('bpm', 0):.1f} BPM)")
    else:
...
async def test_similarity_calculation():
    ...
    context = {
        "suggested_energy_direction": "maintain"
    }
    ...
    print(f"Reference: {reference_track['genre']} @ {reference_track['bpm']} BPM, Energy: {ref_energy:.2f}")
    print(f"Candidate: {candidate_track['genre']} @ {candidate_track['bpm']} BPM, Energy: {cand_energy:.2f}")
