...
class MusicLibraryManager:
                ...
                try:
                    ...
                    if analysis.get('analyzed'):
                        ...
                        if track:
                            ...
                            track.beat_times = json.dumps(analysis.get('beat_times', []))
    ...
    async def search_by_mood(self, mood_description: str, limit: int = 20) -> List[Track]:
        ...
        async with get_session(self.db_engine) as session:
            ...
            if 'energy' in mood_params:
                ...
                query = query.where(
                    and_(
                        Track.energy >= energy_min,
                        Track.energy <= energy_max
                    )
                )
        ...
        async with get_session(self.db_engine) as session:
            ...
            query = select(Track).where(
                and_(
                    ...
                    Track.bpm >= bpm_min,
                    Track.bpm <= bpm_max,
                    or_(*[Track.key == k for k in compatible_keys]) if compatible_keys else True
                )
            )
            ...
            query = query.order_by(
                func.abs(Track.energy - track.energy)
            ).limit(limit)
