...
def create_mock_audio_analysis(
    bpm: float = 128.0, duration: float = 180.0
) -> Dict[str, Any]:
...
def create_mock_file_metadata(
    title: str = "Test Track", artist: str = "Test Artist", duration: float = 180.0
) -> Dict[str, Any]:
...
def create_test_db_track(
    filename: str, bpm: float = 128.0, energy: float = 0.7, **kwargs
) -> Dict[str, Any]:
...
def compare_tracks(track1: Dict[str, Any], track2: Dict[str, Any]) -> Dict[str, float]:
    ...
    return {
        ...
        "compatibility_score": (1 - bpm_diff / 10) * 0.4
        + (1 - energy_diff) * 0.3
        + mood_similarity * 0.3,
    }
