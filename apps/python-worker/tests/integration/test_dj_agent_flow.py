...
from tests.fixtures.mock_data import MOCK_TRACKS_DB, MOCK_DJ_AGENT_THINKING
from tests.utils.helpers import assert_vibe_playlist_valid, create_test_db_track
...
class TestDJAgentFlow:
        ...
        with patch("utils.sqlite_db.get_tracks_by_criteria") as mock_get_tracks:
            ...
            with patch("agents.dj_agent.DJAgent") as mock_dj_agent:
                ...
                response = test_client.post(
                    "/ai/generate-vibe-playlist", json=request_data
                )
                ...
                assert any(
                    "analyze_vibe" in step["step"] for step in data["thinking_process"]
                )
        ...
        with patch("agents.dj_agent.DJAgent") as mock_dj_agent:
            ...
            async def mock_stream():
                steps = [
                    {
                        "step": "analyze_vibe",
                        "output": "Analyzing: dark techno, late night, warehouse",
                    },
                    {
                        "step": "search_tracks",
                        "output": "Searching for tracks with dark, driving energy",
                    },
            ...
            response = test_client.get(
                "/ai/generate-vibe-playlist-stream", params=request_params
            )
    ...
    async def test_dj_agent_energy_patterns(self, test_client, mock_langchain, mock_db):
        ...
        for vibe_desc, expected_pattern in energy_patterns:
            request_data = {"vibe": vibe_desc, "num_tracks": 4, "duration_minutes": 20}
            ...
            with patch("agents.dj_agent.DJAgent") as mock_dj_agent:
                ...
                for i, energy in enumerate(energies):
                    track = create_test_db_track(
                        f"track_{i}.mp3", bpm=125 + i, energy=energy
                    )
                ...
                mock_agent.generate_playlist.return_value = {
                    ...
                    "thinking_process": [
                        {
                            "step": "energy_analysis",
                            "output": f"Using {expected_pattern} pattern",
                        }
                    ],
                }
                mock_dj_agent.return_value = mock_agent
                ...
                if expected_pattern == "build_up":
                    assert all(
                        playlist_energies[i] <= playlist_energies[i + 1]
    ...
    async def test_dj_agent_bpm_progression(self, test_client, mock_langchain):
        ...
        with patch("agents.dj_agent.DJAgent") as mock_dj_agent:
            ...
            for i, bpm in enumerate(bpms):
                track = create_test_db_track(
                    f"progressive_{i}.mp3", bpm=bpm, energy=0.7 + (i * 0.05)
                )
                mock_playlist.append(track)
            ...
            mock_agent.generate_playlist.return_value = {
                ...
                "thinking_process": [
                    {"step": "bpm_analysis", "output": "Target BPM range: 124-128"},
                    {
                        ...
                        "output": "All BPM transitions <= 2 BPM",
                    },
                ],
            }
            mock_dj_agent.return_value = mock_agent
            ...
            for i in range(1, len(playlist)):
                ...
                assert bpm_diff <= 2, (
                    f"BPM jump too large between tracks {i - 1} and {i}"
                )
                ...
                "thinking_process": [
                    {
                        ...
                        "output": "Filtering tracks: BPM 122-126, vocal, non-aggressive",
                    }
                ],
            }
            mock_dj_agent.return_value = mock_agent
