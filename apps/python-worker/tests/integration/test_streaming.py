            ...
            with patch('os.path.getsize', return_value=file_size):
                with patch('builtins.open', mock_open(read_data=fake_data)):
                    ...
                    response = test_client.get(
                        f"/track/{filename}/stream",
                        headers=headers
                    )
                    
                    assert response.status_code == 206  # Partial Content
                    assert response.headers["content-range"] == f"bytes 0-102399/{file_size}"
                    ...
                    assert response.headers["content-range"] == f"bytes 500000-599999/{file_size}"
