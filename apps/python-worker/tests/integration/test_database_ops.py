...
from tests.utils.helpers import create_test_db_track, compare_tracks
...
class TestDatabaseOperations:
    ...
    async def test_store_track_analysis(self, test_client, mock_db, mock_audio_file):
        ...
        from utils.sqlite_db import get_sqlite_db, Track
        
        # Create test data
        track_data = create_test_db_track(
            "test_track.mp3",
            bpm=128.0,
            energy=0.75
        )
        ...
        with patch('utils.sqlite_db.get_sqlite_db') as mock_get_db:
            ...
            # Query back
            cursor.execute("SELECT * FROM tracks WHERE filename = ?", (track_data["filename"],))
    ...
    async def test_beat_times_storage(self, test_client, mock_db):
        ...
        from utils.sqlite_db import get_sqlite_db, update_track_beats
    ...
    async def test_track_search_performance(self, test_client, mock_db):
        ...
        from utils.sqlite_db import get_sqlite_db, search_tracks
    ...
    async def test_database_migrations(self, test_client, mock_db):
        ...
        with patch('utils.sqlite_db.get_sqlite_db') as mock_get_db:
            ...
            required_columns = [
                "id", "filename", "filepath", "title", "artist",
                "bpm", "energy", "duration", "beat_times",
        ...
        with patch('utils.sqlite_db.get_sqlite_db') as mock_get_db:
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?)
                """, ("transaction_test.mp3", "/test/transaction_test.mp3", "Transaction Test"))
            ...
            cursor.execute("SELECT * FROM tracks WHERE filename = ?", ("transaction_test.mp3",))
            result = cursor.fetchone()
    ...
    async def test_duplicate_track_handling(self, test_client, mock_db):
        ...
        from utils.sqlite_db import get_sqlite_db, insert_or_update_track
        ...
        with patch('utils.sqlite_db.get_sqlite_db') as mock_get_db:
            ...
            # Verify update
            cursor.execute("SELECT bpm FROM tracks WHERE filepath = ?", (track["filepath"],))
            result = cursor.fetchone()
            assert result[0] == 125  # BPM should be updated
            
            # Verify no duplicates
            cursor.execute("SELECT COUNT(*) FROM tracks WHERE filepath = ?", (track["filepath"],))
