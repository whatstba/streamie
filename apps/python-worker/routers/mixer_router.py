...
async def update_crossfader(
    ...
    try:
        result = await mixer_manager.update_crossfader(
            request.position, request.apply_to_decks
        )
        if not result["success"]:
            raise HTTPException(
                status_code=400,
                detail=result.get("error", "Failed to update crossfader"),
            )
        return result
    except Exception as e:
...
async def update_crossfader_curve(
    ...
    try:
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to update curve")
            )
        return result
    except Exception as e:
...
async def update_master_output(
    ...
    try:
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to update master")
            )
        return result
    except Exception as e:
...
async def update_monitor_settings(
    request: MonitorRequest, mixer_manager: MixerManager = Depends(get_mixer_manager)
):
    """Update monitor/cue output settings"""
    try:
        result = await mixer_manager.update_monitor_settings(
            request.volume, request.cue_mix
        )
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to update monitor")
            )
        return result
    except Exception as e:
...
async def get_all_channel_levels(
    mixer_manager: MixerManager = Depends(get_mixer_manager),
):
...
async def toggle_deck_cue(
    deck_id: str, mixer_manager: MixerManager = Depends(get_mixer_manager)
):
    ...
    try:
        ...
        return CueToggleResponse(
            success=False, deck_id=deck_id, cue_active=False, error=str(e)
        )
...
async def apply_auto_gain(
    ...
    try:
        ...
        return AutoGainResponse(success=False, deck_id=deck_id, error=str(e))
...
async def start_recording(
    request: RecordingRequest, mixer_manager: MixerManager = Depends(get_mixer_manager)
):
    ...
    try:
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to start recording")
            )
        return result
    except Exception as e:
...
async def stop_recording(mixer_manager: MixerManager = Depends(get_mixer_manager)):
    ...
    try:
        ...
        if not result["success"]:
            raise HTTPException(
                status_code=400, detail=result.get("error", "Failed to stop recording")
            )
        return result
    except Exception as e:
