...
async def get_analysis_status(
    task_id: str, service: AnalysisService = Depends(get_analysis_service)
):
...
async def get_analysis_results(
    request: GetResultsRequest, service: AnalysisService = Depends(get_analysis_service)
):
...
async def analyze_transition(
    ...
    try:
        ...
        result = await agent.analyze_for_transition(
            request.deck_a_id, request.deck_b_id, state
        )
...
async def get_queue_status(service: AnalysisService = Depends(get_analysis_service)):
...
async def stream_analysis(
    ...
    try:
        ...
        await websocket.send_json({"type": "connected", "deck_id": deck_id})
...
async def analyze_batch(
    ...
    try:
        ...
        for filepath in filepaths:
            task_id = await service.enqueue_analysis(
                filepath=filepath, priority=priority, analysis_type="full"
            )
...
async def get_beat_phase(
    request: BeatPhaseRequest, service: AnalysisService = Depends(get_analysis_service)
):
    ...
    try:
        ...
        if not analysis or "beat_times" not in analysis:
            raise HTTPException(
                status_code=404, detail="Beat analysis not available for this track"
            )
        ...
        phase_info = analyzer.analyze_beat_phase(
            request.position, analysis["beat_times"], analysis.get("bpm", 120)
        )
