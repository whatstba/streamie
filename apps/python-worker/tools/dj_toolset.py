...
class DJToolset:
    ...
    async def load_track(
        self, deck: str, file_path: str, track_info: Optional[Dict] = None
    ) -> Dict:
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "deck": deck}
    ...
    def clear_deck(self, deck: str) -> Dict:
        ...
        return {"status": "cleared", "deck": deck}
    ...
    def play_pause(self, deck: str) -> Dict:
        ...
        return {"deck": deck, "playing": state.playing, "position": state.position}
    ...
    def set_crossfader(self, position: float) -> Dict:
        ...
        return {"position": self.mixer.crossfader, "gain_a": gain_a, "gain_b": gain_b}
    ...
    # --- Cues & Loops ---
    def set_cue_point(
        self, deck: str, cue_number: int, position: Optional[float] = None
    ) -> Dict:
        """Set a cue point"""
        state = self.decks[deck]
        if position is None:
        ...
        if deck_state.position_frames + frames_needed >= total_frames:
            ...
            if frames_available > 0:
                ...
                available_audio = audio[
                    :,
                    deck_state.position_frames : deck_state.position_frames
                    + frames_available,
                ]
                resampled = librosa.resample(
                    available_audio, orig_sr=rate, target_sr=1.0, axis=1
                )
            ...
            segment = audio[
                :,
                deck_state.position_frames : deck_state.position_frames + frames_needed,
            ]
