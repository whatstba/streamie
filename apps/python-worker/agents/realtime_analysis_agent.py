...
class RealTimeAnalysisAgent:
    ...
    async def __call__(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        updates = {"analysis_status": {}, "analysis_results": {}}
        ...
        for deck_id, deck_state in decks.items():
            ...
            if self._is_newly_loaded(deck_id, deck_state):
                logger.info(
                    f"🔬 New track detected on deck {deck_id}: {deck_state.get('track_filepath')}"
                )
            ...
            if deck_id in self.active_analyses:
                ...
                if status["status"] == "completed":
                    ...
                    logger.error(
                        f"❌ Analysis failed for deck {deck_id}: {status.get('error')}"
                    )
                    del self.active_analyses[deck_id]
    ...
    async def analyze_for_transition(
        self, deck_a_id: str, deck_b_id: str, state: Dict[str, Any]
    ) -> Dict:
    ...
    def _calculate_transition_compatibility(
        self, analysis_a: Dict, analysis_b: Dict
    ) -> Dict:
    ...
    def _find_transition_points(
        self, analysis_a: Dict, analysis_b: Dict, deck_a: Dict, deck_b: Dict
    ) -> List[Dict]:
        ...
        if beats_a and beats_b:
            ...
            bars_16_a = [
                beats_a[i] for i in range(0, len(beats_a), 64) if i < len(beats_a)
            ]
            bars_16_b = [
                beats_b[i] for i in range(0, len(beats_b), 64) if i < len(beats_b)
            ]
    ...
    def _suggest_transition_effects(
        self, analysis_a: Dict, analysis_b: Dict, compatibility: Dict
    ) -> List[Dict]:
        ...
        # Energy difference
        energy_diff = abs(
            analysis_a.get("energy_level", 0.5) - analysis_b.get("energy_level", 0.5)
        )
        if energy_diff > 0.3:
            effects.append(
                {
