...
def test_analysis_endpoints():
    ...
    data = {"filepath": filepath, "priority": 1, "analysis_type": "realtime"}
    ...
    response = requests.post(
        f"{BASE_URL}/api/analysis/results", json={"filepath": filepath}
    )
    print(f"Status: {response.status_code}")
    if response.status_code == 200:
        result = response.json()
    ...
    response = requests.post(f"{BASE_URL}/api/analysis/beat-phase", json=data)
    print(f"Status: {response.status_code}")
    if response.status_code == 200:
        print(f"Response: {json.dumps(response.json(), indent=2)}")
...
def test_deck_analysis_integration():
    ...
    data = {
        "track_filepath": "../../../../Downloads/PLAYLISTS/2000 FEELGOODS/Ashanti/Concrete Rose (Album)/04 Only U.mp3"
    }
