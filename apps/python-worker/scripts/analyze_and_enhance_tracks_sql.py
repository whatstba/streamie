...
class TrackDatabase:
    ...
    def needs_analysis(self, file_path: str) -> bool:
        ...
        cursor.execute(
            "SELECT file_hash, last_modified FROM tracks WHERE filepath = ?",
            (rel_path,),
        )
        row = cursor.fetchone()
        ...
        return (
            ...
            or abs(current_mtime - row["last_modified"]) > 1
        )
...
class TrackAnalyzer:
    ...
    def enhance_track_features(
        self, file_path: str, bpm: float, beat_times: List[float], mood: Dict
    ) -> Dict:
