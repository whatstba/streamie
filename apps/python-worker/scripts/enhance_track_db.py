...
def detect_vocal_presence(file_path: str) -> float:
    ...
    try:
        ...
        vocal_range_ratio = np.sum(
            (spectral_centroids > 300) & (spectral_centroids < 3000)
...
def enhance_track_metadata():
    ...
    for i, track in enumerate(tracks):
        ...
        energy_level = calculate_energy_level(
            track.get("bpm", 120), track.get("mood", {})
        )

        # Estimate danceability
        danceability = estimate_danceability(
            track.get("bpm", 120), track.get("beat_times", [])
        )

        # Calculate tempo stability
        tempo_stability = calculate_tempo_stability(track.get("beat_times", []))
        ...
        update_data = {
            ...
            "valence": track.get("mood", {}).get(
                "mood_happy", 0.5
            ),  # Use happy mood as proxy for valence
        }

        collection.update_one({"_id": track["_id"]}, {"$set": update_data})

        print(
            f"  Energy: {energy_level:.2f}, Dance: {danceability:.2f}, "
