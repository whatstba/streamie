"""
Utility script to query and manage the SQLite tracks database.

Usage examples:
  python query_tracks_db.py --stats               # Show database statistics
  python query_tracks_db.py --search "artist"     # Search for tracks by artist
  python query_tracks_db.py --bpm-range 120 140   # Find tracks in BPM range
  python query_tracks_db.py --high-energy         # Find high energy tracks
  python query_tracks_db.py --export tracks.json  # Export all tracks to JSON
"""
...
class TrackQuery:
    ...
    def __init__(self, db_path: str = DB_PATH):
        ...
        if not os.path.exists(db_path):
            ...
            print("Run analyze_and_enhance_tracks_sql.py first to create the database.")
            sys.exit(1)
    ...
    def get_stats(self) -> Dict:
        ...
        cursor.execute(
            "SELECT COUNT(DISTINCT artist) as artists FROM tracks WHERE artist IS NOT NULL"
        )
        total_artists = cursor.fetchone()["artists"]

        cursor.execute(
            "SELECT COUNT(DISTINCT album) as albums FROM tracks WHERE album IS NOT NULL"
        )
        total_albums = cursor.fetchone()["albums"]

        # BPM stats
        cursor.execute(
            "SELECT AVG(bpm) as avg_bpm, MIN(bpm) as min_bpm, MAX(bpm) as max_bpm FROM tracks WHERE bpm IS NOT NULL"
        )
        bpm_stats = cursor.fetchone()

        # Energy stats
        cursor.execute(
            "SELECT AVG(energy_level) as avg_energy, MIN(energy_level) as min_energy, MAX(energy_level) as max_energy FROM tracks WHERE energy_level IS NOT NULL"
        )
        energy_stats = cursor.fetchone()

        # Duration stats
        cursor.execute(
            "SELECT SUM(duration) as total_duration FROM tracks WHERE duration IS NOT NULL"
        )
        ...
        return {
            ...
            "top_moods": [
                {"mood": row["mood_label"], "count": row["count"]} for row in top_moods
            ],
        }
    ...
    def get_similar_tracks(
        self, reference_track_id: int, limit: int = 10
    ) -> List[Dict]:
        ...
        cursor.execute(
            ...
            SELECT *,
                   ABS(bpm - ?) * 0.3 +
                   ABS(energy_level - ?) * 0.25 +
                   ABS(danceability - ?) * 0.25 +
                   ABS(valence - ?) * 0.2 as similarity_score
            FROM tracks 
        ...
        """,
            (
                ref_track["bpm"],
                ref_track["energy_level"],
                ref_track["danceability"],
                ref_track["valence"],
                reference_track_id,
                limit,
            ),
...
def print_track_summary(track: Dict):
    ...
    print(
        f"     BPM: {track['bpm']:.1f}, Energy: {track['energy_level']:.2f}, "
        f"Dance: {track['danceability']:.2f}, Mood: {track['mood_label'] or 'Unknown'}"
    )
    print()
...
def main():
    ...
    parser.add_argument(
        "--search", metavar="QUERY", help="Search tracks by artist/title/album/genre"
    )
    parser.add_argument(
        "--bpm-range",
        nargs=2,
    ...
    parser.add_argument(
        "--high-energy", action="store_true", help="Find high energy tracks (>= 0.7)"
    )
    parser.add_argument(
        "--danceable", action="store_true", help="Find highly danceable tracks (>= 0.7)"
    )
    parser.add_argument("--mood", metavar="MOOD", help="Find tracks by mood")
    parser.add_argument(
        "--similar",
        metavar="TRACK_ID",
        type=int,
        help="Find tracks similar to given track ID",
    )
    parser.add_argument(
        "--export", metavar="FILENAME", help="Export all tracks to JSON file"
    )
    parser.add_argument(
    ...
    try:
        if args.stats:
            ...
            print()
            print(
                f"BPM - Avg: {stats['bpm']['avg']:.1f}, "
                f"Range: {stats['bpm']['min']:.1f}-{stats['bpm']['max']:.1f}"
            )
            print(
                f"Energy - Avg: {stats['energy']['avg']:.2f}, "
                f"Range: {stats['energy']['min']:.2f}-{stats['energy']['max']:.2f}"
            )
            print()
            print("Top moods:")
            for mood in stats["top_moods"]:
                print(f"  {mood['mood']}: {mood['count']} tracks")
            ...
            tracks = db.get_tracks_by_mood(args.mood)
            print(f"😊 Tracks with mood '{args.mood}' ({len(tracks)} tracks):")
            print()
            for track in tracks[: args.limit]:
                print_track_summary(track)
            ...
            if tracks:
                print(
                    f"🎯 Tracks similar to track ID {args.similar} ({len(tracks)} tracks):"
                )
                print()
                for track in tracks:
                    print_track_summary(track)
